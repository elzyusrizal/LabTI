...
# ==========================================
# SOAL 2 - Permintaan API dan Penanganannya
# ==========================================
def request_stock_price_from_api(symbol, current_thread_name):
    ...
    log_client_activity_safe(current_thread_name, f"Mengirim permintaan untuk simbol: {symbol} ke {target_url}")
    try:
        ...
        if response.status_code == 200:
            ...
            log_client_activity_safe(
                current_thread_name,
                f"Berhasil! Saham {data.get('symbol', symbol)}: Harga {data.get('price', 'N/A')}, "
                f"Perubahan {data.get('change', 'N/A')}, Volume {data.get('volume', 'N/A')}"
            )
        elif response.status_code == 404:
            data = response.json()
            log_client_activity_safe(
                current_thread_name,
                f"Error: Simbol {symbol} tidak ditemukan. Pesan: {data.get('message', 'Not found')}"
            )
        else:
            log_client_activity_safe(
                current_thread_name,
        ...
        log_client_activity_safe(current_thread_name, f"Permintaan timeout untuk simbol {symbol}.")
    except requests.exceptions.RequestException as e:
        log_client_activity_safe(current_thread_name, f"Permintaan gagal untuk simbol {symbol}. Error: {str(e)}")
    except Exception as e:
        log_client_activity_safe(current_thread_name, f"Kesalahan tak terduga untuk simbol {symbol}: {str(e)}")
    log_client_activity_safe(current_thread_name, f"Tugas selesai untuk simbol: {symbol}")
