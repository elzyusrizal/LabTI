...
def client_cek_saldo_via_api(nomor_rekening, thread_name):
    ...
    print(f"[{thread_name}] Meminta saldo untuk rekening: {nomor_rekening}")
    try:
        response = requests.get(target_url, timeout=5)
        if response.status_code == 200:
            data = response.json()
            print(f"[{thread_name}] Saldo {nomor_rekening} ({data.get('nama_pemilik')}): {data.get('saldo')}")
        elif response.status_code == 404:
            print(f"[{thread_name}] Rekening {nomor_rekening} tidak ditemukan.")
        else:
            print(f"[{thread_name}] Error API untuk {nomor_rekening}: Status {response.status_code}")
    except requests.exceptions.Timeout:
        print(f"[{thread_name}] Permintaan timeout untuk rekening {nomor_rekening}.")
    except requests.exceptions.RequestException as e:
        print(f"[{thread_name}] Permintaan gagal untuk rekening {nomor_rekening}. Error: {str(e)}")
